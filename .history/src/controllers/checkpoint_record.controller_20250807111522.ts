...
export class CheckpointRecordController {
  ...
  async findByIdWithFullInfo(req: Request, res: Response): Promise<void> {
    try {
      const id = parseInt(req.params.id);
      const result = await this.checkpointRecordService.findByIdWithFullInfo(id);
      res.status(200).json({
        message: "Registro de checkpoint con información completa obtenido exitosamente",
        data: result,
      });
    } catch (error) {
      res.status(404).json({
        message: error instanceof Error ? error.message : "Error al obtener el registro de checkpoint con información completa",
      });
    }
  }

  async findAllByPatrolAssignmentId(req: Request, res: Response): Promise<void> {
    try {
      const patrolAssignmentId = parseInt(req.params.patrolAssignmentId);
      const result = await this.checkpointRecordService.findAllByPatrolAssignmentId(patrolAssignmentId);
      res.status(200).json({
        message: "Registros de checkpoint por asignación obtenidos exitosamente",
        data: result,
      });
    } catch (error) {
      res.status(400).json({
        message: error instanceof Error ? error.message : "Error al obtener los registros de checkpoint por asignación",
      });
    }
  }
